...
import { api, qk, type MaquinaDTO } from "@/lib/queries";
import Link from "next/link";

function turnosDe(m: MaquinaDTO) {
  return [
...
export function TurnosClient() {
  ...
  const maquinaParam = sp.get("maquina");
  const { data: maquinas, isLoading } = useQuery({ queryKey: qk.maquinas, queryFn: api.maquinas });
  const m = maquinas?.find((x) => x.id === maquinaParam) ?? maquinas?.[0];
      ...
      <Panel title="Máquina">
        {isLoading ? (
          ...
          <div className="flex flex-wrap gap-2">
            {(!maquinas || maquinas.length === 0) && <Empty>Nenhuma máquina — <Link href="/maquinas" className="text-blue underline">cadastre primeiro</Link>.</Empty>}
            {maquinas?.map((x) => {
      ...
      {m && (
        <>
          <Panel className="mt-5" title={`Turnos — ${m.codigo}`} desc="Deixe início/fim em branco para turno inativo. Eficiência desconta micro-paradas e refugo.">
            <ActionForm key={m.id} action={salvarTurnos} submitLabel="Salvar calendário" resetOnSuccess={false} invalidate={[qk.maquinas]}>
              <input type="hidden" name="id" value={m.id} />
              <FormGrid cols={3}>
                {[1, 2, 3].map((t) => {
                  const ini = (m as unknown as Record<string, unknown>)[`turno${t}Inicio`] as string | null;
                  const fim = (m as unknown as Record<string, unknown>)[`turno${t}Fim`] as string | null;
                  return (
          ...
          <Panel className="mt-5" title="Paradas programadas" desc="Manutenção preventiva, feriado, troca de equipamento — descontadas da capacidade da semana em que ocorrem.">
            <ActionForm key={`p-${m.id}`} action={adicionarParada} submitLabel="+ Adicionar parada" invalidate={[qk.maquinas]}>
                ...
                <tbody>
                  {m.paradas.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td><Tag kind="neutral">{fmt.h((new Date(p.fim).getTime() - new Date(p.inicio).getTime()) / 3600000)}</Tag></td>
                      <td><InlineAction action={removerParada} label="Remover" danger hidden={{ id: p.id }} invalidate={[qk.maquinas]} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
