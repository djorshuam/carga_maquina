...
function inicioSemanaIso(base: Date): string {
...
export function HomeClient() {
  ...
  const semanaIso = inicioSemanaIso(isNaN(base.getTime()) ? new Date() : base);

  const { data, isLoading } = useQuery({ queryKey: qk.carga(semanaIso), queryFn: () => api.carga(semanaIso) });
  const linhas = data?.linhas ?? [];
  const semana = new Date(data?.semana ?? semanaIso + "T00:00:00Z");
  ...
  const shift = (days: number) => {
    const d = new Date(semana.getTime() + days * 86400000);
    router.push(`/?semana=${d.toISOString().slice(0, 10)}`);
  };
    ...
    <>
      ...
      <Panel title="Gantt semanal por máquina" desc="Azul = ordem · Cinza = setup (troca de molde) · Vermelho = excede a capacidade disponível do período. Ordens sequenciadas por prioridade e prazo.">
        <div className="flex items-center gap-2 mb-3 text-[12px]">
          <button className="btn !py-1 !px-2 !text-[12px]" onClick={() => shift(-7)}>← semana anterior</button>
          <button className="btn !py-1 !px-2 !text-[12px]" onClick={() => router.push("/")}>hoje</button>
          <button className="btn !py-1 !px-2 !text-[12px]" onClick={() => shift(7)}>próxima semana →</button>
        </div>
        {isLoading ? (
          <div className="text-[13px] text-muted py-6 text-center">Carregando…</div>
        ) : linhas.length === 0 ? (
          <Empty>Nenhuma máquina cadastrada. Comece em <Link href="/maquinas" className="text-blue underline">Cadastros → Máquinas</Link>.</Empty>
        ) : (
          <Gantt linhas={linhas} semana={semana} />
        )}
      </Panel>
